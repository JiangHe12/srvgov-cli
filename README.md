...
- **Fail-closed read audit** — governed R0 reads persist an `intent` before backend access and an `outcome` before any result is released. If either required record cannot be made durable, the read returns `LOCAL_IO_ERROR`; backend output is withheld.
